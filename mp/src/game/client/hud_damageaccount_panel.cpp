...
ConVar la_cl_damagepopup("la_cl_damagepopup", "1", FCVAR_ARCHIVE, "Enable damage popups (client option)");
ConVar la_cl_damagepopup_batching("la_cl_damagepopup_batching", "1", FCVAR_ARCHIVE, "If set to 1, numbers that are too close together are merged.");
ConVar la_cl_damagepopup_batching_window("la_cl_damagepopup_batching_window", "0.2", FCVAR_ARCHIVE, "Maximum delay between damage events in order to batch numbers.");
...
void CDamageAccountPanel::OnDamaged(IGameEvent *event)
{
	...
	if (pPlayer && pPlayer->IsAlive())
	{
		...
		if (!la_cl_damagepopup.GetBool() || !la_sv_damagepopup.GetBool())
			return;
		...
		if (la_cl_damagepopup_batching.GetBool())
		{
			...
			for (int i = 0; i < NUM_ACCOUNT_DELTA_ITEMS; i++)
			{
				if (m_AccountDeltaItems[i].m_hEntity.Get() == pVictim)
				{
					...
					if (gpGlobals->curtime - flCreateTime < la_cl_damagepopup_batching_window.GetFloat())
					{
